 ...
 * </p>
 */
public final class AsymmetricUncenteredCosineSimilarity extends
		AbstractSimilarity {
	private boolean divideByX = false;
	private boolean divideByY = false;

	public void setDivideByX(boolean divideByX) {
		this.divideByX = divideByX;
	}

	public void setDivideByY(boolean divideByY) {
		this.divideByY = divideByY;
	}
	 ...
	 */
	public AsymmetricUncenteredCosineSimilarity(DataModel dataModel)
	...
	public AsymmetricUncenteredCosineSimilarity(DataModel dataModel,
		...
		System.out.println("instantiating asymcos");
	...
	double computeResult(int n, double sumXY, double sumX2, double sumY2,
		...
		if (divideByX)
			denominator *= Math.sqrt(sumX2);
		if (divideByY)
			denominator *= Math.sqrt(sumY2);
		if (denominator == 0.0) {
			// One or both parties has -all- the same ratings;
			// can't really say much similarity under this measure
			return Double.NaN;
		}
		double result = sumXY / denominator;
