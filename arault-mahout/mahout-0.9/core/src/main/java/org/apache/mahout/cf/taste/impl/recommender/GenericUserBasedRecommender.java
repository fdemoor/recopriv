...
public class GenericUserBasedRecommender extends AbstractRecommender implements UserBasedRecommender {
  ...
  public List<RecommendedItem> recommend(long userID, int howMany, String choiceBehavior, Random rand) throws TasteException {
  Preconditions.checkArgument(howMany >= 1, "howMany must be at least 1");

    log.debug("Recommending items for user ID '{}'", userID);

    long[] theNeighborhood = neighborhood.getUserNeighborhood(userID, choiceBehavior, rand);

    // Debug
    /*StringBuilder builder = new StringBuilder();*/
    //builder.append("User ");
    //builder.append(userID);
    //builder.append("'s neighbors for recommendation are: [");
    ...
    // Debug
    //System.out.println("allItemIDs' size = "+allItemIDs.size());
  ...
  protected float doEstimatePreference(long theUserID, long[] theNeighborhood, long itemID) throws TasteException {
    ...
    // Local change: do not throw out estimate anymore if based on 1 rating.
    if (count < 1) {
      // Debug
      //System.out.println("Discarded item "+itemID+"'s estimate because based on 0 or 1 ratings");
      return Float.NaN;
    }
