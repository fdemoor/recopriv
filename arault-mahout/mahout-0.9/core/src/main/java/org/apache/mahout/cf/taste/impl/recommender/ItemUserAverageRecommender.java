...
public final class ItemUserAverageRecommender extends AbstractRecommender {
  ...
  private final FastByIDMap<LongRunningAverage> itemAverages;
  private final FastByIDMap<LongRunningAverage> userAverages;
  private final LongRunningAverage overallAveragePrefValue;
  ...
  public ItemUserAverageRecommender(DataModel dataModel) throws TasteException {
    super(dataModel);
    this.itemAverages = new FastByIDMap<LongRunningAverage>();
    this.userAverages = new FastByIDMap<LongRunningAverage>();
    this.overallAveragePrefValue = new FullLongRunningAverage();
  ...
  private float doEstimatePreference(long userID, long itemID) {
    ...
    try {
      LongRunningAverage itemAverage = itemAverages.get(itemID);
      if (itemAverage == null) {
        return Float.NaN;
      }
      LongRunningAverage userAverage = userAverages.get(userID);
      if (userAverage == null) {
        return Float.NaN;
      }
      double userDiff = userAverage.getAverage() - overallAveragePrefValue.getAverage();
      log.debug("User's avg={}, global avg={}, diff={}", userAverage.getAverage(), overallAveragePrefValue.getAverage(), userDiff);
      log.debug("Item's avg={}, item's avg+diff={}", itemAverage.getAverage(), itemAverage.getAverage()+userDiff);
  ...
  private void buildAverageDiffs() throws TasteException {
    try {
	  ...
	  log.debug("User:{}, item:{}, rating={}, nb ratings={}, global avg={}", userID, itemID, value, overallAveragePrefValue.getCount(), overallAveragePrefValue.getAverage());
        }
      }
    } finally {
  ...
  private static void addDatumAndCreateIfNeeded(long itemID, float value, FastByIDMap<LongRunningAverage> averages) {
    LongRunningAverage itemAverage = averages.get(itemID);
    if (itemAverage == null) {
      itemAverage = new FullLongRunningAverage();
  ...
  public void setPreference(long userID, long itemID, float value) throws TasteException {
    ...
    try {
      ...
      LongRunningAverage itemAverage = itemAverages.get(itemID);
      if (itemAverage == null) {
        LongRunningAverage newItemAverage = new FullLongRunningAverage();
      ...
      LongRunningAverage userAverage = userAverages.get(userID);
      if (userAverage == null) {
        LongRunningAverage newUserAveragae = new FullLongRunningAverage();
