...
/**
 * Abstract superclass of a couple implementations, providing shared functionality which runs sequentially.
 */
public abstract class AbstractDifferenceRecommenderSequentialEvaluator implements RecommenderEvaluator {
  
  private static final Logger log = LoggerFactory.getLogger(AbstractDifferenceRecommenderSequentialEvaluator.class);
  
  private Random random;
  private float maxPreference;
  private float minPreference;
  
  protected AbstractDifferenceRecommenderSequentialEvaluator() {
  ...
  protected static void execute(Collection<Callable<Void>> callables,
    ...
    log.info("Starting timing of {} tasks in 1 thread", wrappedCallables.size());
    try {
  ...
  /**
   * Set the seed of the PRNG so that results are reproducible (if executed sequentially).
   */
  public void setReproducibilitySeed(long seed) {
