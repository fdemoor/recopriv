...
##################################################################################
# Randomly chosen neighbors, non-adaptive Sybil attack, one target attacked at a time, with cosine on jester #
##################################################################################
...
expeDir="jester/non-adaptive/one_target_at_a_time/${neighborhoodType}_neighbors/${similarityType}"
mkdir -p $expeDir
cd $expeDir
...
for targetID in $(seq 1 24983); do
	...
	for percentAuxiliaryItems in 0.25 0.75; do
...
for targetID in $(seq 1 24983); do
	for percentAuxiliaryItems in 0.2 0.3 0.7 0.8; do
		currentExpe="percentAuxItems_${percentAuxiliaryItems}"
		mkdir -p $currentExpe
		cd $currentExpe
