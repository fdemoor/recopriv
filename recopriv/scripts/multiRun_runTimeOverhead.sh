...
r=10
computeAboveThres=false
computeBuildRunTime=true
computeBuildMemory=false
doSybilAttack=false
nbAttackers=1
nbTargets=1
targetID=13
...
expeDir="ml100k/Overhead"
mkdir -p $expeDir
cd $expeDir


function runSimu {
	currentExpe="${similarityType}"
	mkdir -p $currentExpe
	cd $currentExpe
	SIMULATION_PARAMETERS=$($BASEDIR/scripts/generateParametersString.sh)
...
for ((index=0; index<10; index++)) do
	for similarityType in "cosine" "twostep(0.8)"; do
		runSimu &
	done
	wait
done
