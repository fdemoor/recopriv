...
##################################################################################
# Non-adaptive Sybil attack, one target attacked at a time, with cosine-mahout on jester-alt #
##################################################################################
...
datasetPath=$baseDir/datasets/jester/jester-data-1_no_universal_items.csv
similarityType=cosine-mahout

expeDir="jester-alt/non-adaptive/one_target_at_a_time/${similarityType}"
mkdir -p $expeDir
cd $expeDir
...
for targetID in $(seq 1 24983); do
	for percentAuxiliaryItems in 0.2 0.5 0.8 0.1 0.3 0.7 0.9; do
