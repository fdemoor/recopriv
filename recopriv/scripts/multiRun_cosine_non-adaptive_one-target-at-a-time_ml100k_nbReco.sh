...
##################################################################################
# Non-adaptive Sybil attack, one target attacked at a time, with cosine on ml100k, checking the effect of the number of reco asked on accuracy  #
##################################################################################
...
likeThreshold=3
doOnlyPrecomputations=false
percentAuxiliaryItems=0.8

expeDir="ml100k/non-adaptive/one_target_at_a_time/${similarityType}/percentAuxItems_${percentAuxiliaryItems}"
mkdir -p $expeDir
cd $expeDir

for targetID in $(seq 1 943); do
	for sybilsNbRecoPerRound in 1 2 3 5 7 8 9 10 12 15 18 20 22 25 27 30 35 40 50; do
		currentExpe="nbReco_${sybilsNbRecoPerRound}"
		mkdir -p $currentExpe
		cd $currentExpe
