...
##################################################################################
# Adaptive Sybil attack, one target attacked at a time, with wupXontoY on movietweetings #
##################################################################################
...
datasetPath=$baseDir/datasets/movietweetings_recsys_challenge_2014/training_test_eval_concat_cleaned.csv
similarityType=wupXontoY

expeDir="movietweetings/adaptive/one_target_at_a_time/${similarityType}"
mkdir -p $expeDir
cd $expeDir
...
sybilsNbRecoPerRound=1
nbTargets=1
likeThreshold=5

for targetID in $(seq 1 24921); do
	for percentAuxiliaryItems in 0.2 0.5 0.8; do
...
for targetID in $(seq 1 24921); do
	for percentAuxiliaryItems in 0.1 0.3 0.7 0.9; do
		currentExpe="percentAuxItems_${percentAuxiliaryItems}"
		mkdir -p $currentExpe
		cd $currentExpe
