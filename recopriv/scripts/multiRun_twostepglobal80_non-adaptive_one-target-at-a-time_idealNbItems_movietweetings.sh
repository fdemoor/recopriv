...
##############################################################################################################
# Non-adaptive Sybil attack, one target attacked at a time, with twostepglobal80 measures and varying idealNbItems #
##############################################################################################################
...
nbTargets=1
likeThreshold=5
similarityType="twostepglobal(0.8)"

expeDir="movietweetings/non-adaptive/one_target_at_a_time/${similarityType}/"
mkdir -p $expeDir
cd $expeDir

for targetID in $(seq 1 24921); do
	for twostepIdealNbItems in 10 25 50 100 200 400 500; do 
