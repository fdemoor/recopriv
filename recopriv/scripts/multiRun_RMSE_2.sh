...
expeDir="MovieTweetings/KFoldRMSE"
mkdir -p $expeDir
cd $expeDir
...
function runSimu {
	currentExpe="${similarityType}/k_${k}"
	mkdir -p $currentExpe
	cd $currentExpe
	SIMULATION_PARAMETERS=$($BASEDIR/scripts/generateParametersString.sh)
...
for similarityType in "cosine-mahout" "jaccard" "pearson" "wupXontoY" "wupYontoX"; do
	for k in 1 3 5 10 15 20 25 30 40 50 75 100; do
		runSimu &
	done
	wait
done
